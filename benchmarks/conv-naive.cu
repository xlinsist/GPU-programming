...
const float OUTPUT_RESULT_VALUE = 50.0f;
...
void __global__ execute(const float *input, float *output, const int n)
{
    ...
    for (int i = 0; i < KERNEL_SIZE; i++)
        for (int j = 0; j < KERNEL_SIZE; j++)
        {
            ...
            if (inCol >= 0 && inCol < n && inRow >= 0 && inRow < n)
                sum += KERNEL[i][j] * input[inRow * n + inCol];
        }
    if (outCol >= 0 && outCol < n - KERNEL_SIZE + 1 && outRow >= 0 && outRow < n - KERNEL_SIZE + 1)
        output[outRow * (n - KERNEL_SIZE + 1) + outCol] = sum;
}

int main(void)
{
    const int numInputBytes = sizeof(float) * INPUT_SIZE * INPUT_SIZE;
    const int numKernelBytes = sizeof(float) * KERNEL_SIZE * KERNEL_SIZE;
    const int numOutputBytes = sizeof(float) * OUTPUT_SIZE * OUTPUT_SIZE;
    ...
    for (int repeat = 1; repeat <= NUM_REPEATS; ++repeat)
    {
        ...
        execute<<<gridSize, blockSize>>>(d_x, d_z, INPUT_SIZE);
