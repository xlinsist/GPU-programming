...
__constant__ float FC2_WEIGHT[FC2][FC1];
__constant__ float FC3_WEIGHT[FC3][FC2];
...
void __global__ FCProcessing2(const float *input, float *output)
{
    ...
    for (int i = 0; i < FC1; i++)
    {
        sum += input[batch * FC1 + i] * FC2_WEIGHT[t][i];
    }
    sum += FC2_BIAS[t];
    if (sum < 0.0f)
        sum = 0.0f;
...
void __global__ FCProcessing3(const float *input, int *prediction)
{
    ...
    float sum = 0.0f;
    for (int i = 0; i < FC2; i++)
        sum += input[batch * FC2 + i] * FC3_WEIGHT[t][i];
    output_s[t] = sum;
...
int main(int argc, char* argv[]) {
    ...
    cudaMemcpyToSymbol(FC2_WEIGHT, fc2_weight.data(), FC2 * FC1 * sizeof(float));
    cudaMemcpyToSymbol(FC3_WEIGHT, fc3_weight.data(), FC3 * FC2 * sizeof(float));
    ...
    for (int t = 0; t < TOTAL_1; t += BATCH_SIZE) {
        ...
        FCProcessing2<<<BATCH_SIZE, FC2>>>(fc1Output_d, fc2Output_d);
        ...
        FCProcessing3<<<BATCH_SIZE, FC3>>>(fc2Output_d, fc3Output_d);
    ...
    //     cudaMalloc((void **)&fc2Output_d, fc2OutputBytes);
    //     FCProcessing2<<<BATCH_SIZE_2, FC2>>>(fc1Output_d, fc2Output_d);
    ...
    //     int* prediction = (int*) malloc(fc3OutputBytes);
    //     cudaMalloc((void**)&fc3Output_d, fc3OutputBytes);
    //     FCProcessing3<<<BATCH_SIZE_2, FC3>>>(fc2Output_d, fc3Output_d);
    //     cudaMemcpy(prediction, fc3Output_d, fc3OutputBytes, cudaMemcpyDeviceToHost);
    //     for (int i = 0; i < BATCH_SIZE_2; i++)
    //         if (prediction[i] == labels[t + i])
    //             correct++;
