...
void __global__ ConvProcessing1(const float *input, float *output)
{
    ...
    const int outCol = tx;
    const int outRow = ty;
    __shared__ float output_s[BATCH_SIZE][F1][CONV1_OUTPUT_SIZE][CONV1_OUTPUT_SIZE];
    ...
    if (outCol < CONV1_OUTPUT_SIZE && outRow < CONV1_OUTPUT_SIZE)
    {
        ...
        {    
            ...
            if (tmp < 0.0f)
                tmp = 0.0f;
            output_s[batch][k][outRow][outCol] = tmp;
            __syncthreads();
            ...
            if ((outCol & 1) && (outRow & 1))
            {
                ...
                float tmp1 = output_s[batch][k][outRow][outCol] > output_s[batch][k][outRow][outCol - 1] ? 
                    output_s[batch][k][outRow][outCol] : output_s[batch][k][outRow][outCol - 1];
                float tmp2 = output_s[batch][k][outRow - 1][outCol] > output_s[batch][k][outRow - 1][outCol - 1] ? 
                    output_s[batch][k][outRow - 1][outCol] : output_s[batch][k][outRow - 1][outCol - 1];
...
void __global__ ConvProcessing2(const float *input, float *output)
{
    ...
    const int outCol = tx;
    const int outRow = ty;
    __shared__ float output_s[BATCH_SIZE][F2][CONV2_OUTPUT_SIZE][CONV2_OUTPUT_SIZE];
                ...
                for (int j = 0; j < KERNEL_SIZE; j++)
                {
                    ...
                    for (int l = 0; l < C2; l++)
                    {
                        sum += CONV2_WEIGHT[k][l][i][j] * 
                            input[batch * POOL1_OUTPUT_SIZE * POOL1_OUTPUT_SIZE * C1 +
...
{
    __shared__ float output_s[BATCH_SIZE][FC3];
    const int batch = blockIdx.x;
    const int t = threadIdx.x;
    float sum = 0.0f;
    for (int i = 0; i < FC2; i++)
        sum += input[batch * FC2 + i] * weight[t * FC2 + i];
    output_s[batch][t] = sum;

    __syncthreads();  
    if (t == 0) 
    {
        float bestResult = output_s[batch][0];
        int bestId = 0;
        for (int i = 1; i < FC3; i++)
            if (output_s[batch][i] > bestResult)
            {
                bestResult = output_s[batch][i];
                bestId = i;
            }
...
int main(int argc, char* argv[]) {
    ...
    int correct = 0;
    for (int t = 0; t < TOTAL; t+=BATCH_SIZE) {
        ...
        CHECK(cudaMemcpy(prediction, fc3Output_d, fc3OutputBytes, cudaMemcpyDeviceToHost));
